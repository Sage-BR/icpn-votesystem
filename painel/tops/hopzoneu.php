...
if ($response === false || $http_status != 200) {
    ...
    if ($json !== null && isset($json['status']) && $json['status'] === 'completed') {
        if (isset($json['vote_time'])) {
            ...
            date_default_timezone_set('Europe/Athens'); // Exemplo de GMT+3
            ...
            if ($current_time >= $next_vote_time) {
                $can_vote = true;
            } else {
                $can_vote = false;
            }
...
if ($can_vote):
    ...
    <script language="javascript">
        atualizaContador(<?php echo $row->id; ?>,<?php echo date('Y', strtotime($next_vote_time)); ?>,<?php echo date('m', strtotime($next_vote_time)); ?>,<?php echo date('d', strtotime($next_vote_time)); ?>,<?php echo date('H', strtotime($next_vote_time)); ?>,<?php echo date('i', strtotime($next_vote_time)); ?>,<?php echo date('s', strtotime($next_vote_time)); ?>);
    </script>
