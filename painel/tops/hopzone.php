...
if (@fsockopen(str_replace("https://", "", str_replace("http://", "", $row->top_url)), 80, $errno, $errstr, 30)) {
    ...
    // Verifica se o JSON foi decodificado corretamente e se "voted" é verdadeiro
    if (!empty($pagina) && $pagina->voted === true && !empty($pagina->voteTime) && !empty($pagina->hopzoneServerTime)) {
        // Calcula o tempo permitido para o próximo voto
        $voteTime = strtotime($pagina->voteTime); // Hora da votação
        $hopzoneServerTime = strtotime($pagina->hopzoneServerTime); // Hora atual do servidor
        $nextVoteTime = strtotime("+12 hours", $voteTime); // Próximo horário permitido para votar

        // Verifica se o próximo voto ainda não é permitido
        if ($nextVoteTime > $hopzoneServerTime) {
            ...
            <div style='width:87px; height:47px; border:1px solid #999; margin-top:5px; margin-left:5px; float:left;'>
                <a href='https://vgw.hopzone.net/site/vote/<?php echo $row->top_id; ?>/1' target='_blank'>
        ...
        // Se a API não retornar "voted: true" ou se houver erro no JSON
